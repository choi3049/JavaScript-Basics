...
// 기다리지 않고 다음 코드를 먼저 수행하는 자바스크립트의 특성’을 의미
...
//   응답이 없으면 reject() 메서드를 호출하는 예제입니다. 호출된 메서드에 따라
//   then()이나 catch()로 분기하여 응답 결과 또는 오류를 출력합니다.

// 프로미스의 또 다른 특징은 여러 개의 프로미스를 연결하여 사용할 수 있다는 점입니다.
// 앞 예제에서 then() 메서드를 호출하고 나면 새로운 프로미스 객체가 반환됩니다. 따라서,
// 아래와 같이 코딩이 가능합니다.
function getData() {
  return new Promise({
    // ...
...
//   각각 프로미스를 반환해주는 함수라고 가정했습니다. 아래와 같이 말이죠.
...
// 개개인의 코딩 스타일에 따라서 then()의 두 번째 인자로 처리할 수도 있고 catch()로 처리할 수도 있겠지만
// 가급적 catch()로 에러를 처리하는 게 더 효율적입니다.
